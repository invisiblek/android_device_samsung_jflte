...
if [ $? = 0 ];
    then
       ...
       #customize build.prop
       sed -e 's|ro.build.fingerprint=custom|ro.build.fingerprint=samsung/jfltevl/jfltecan:4.2.2/JDQ39/I337MVLUAMDJ:user/release-keys|g' build.prop > build.tmp
       sed -e 's|ro.build.description=custom|ro.build.description=jfltevl-user 4.2.2 JDQ39 I337MVLUAMDJ release-keys|g' build.tmp > build.prop
